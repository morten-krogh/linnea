...
| `proxy_sni` | string | none | ≤ 255 | The server name to send in the TLS ClientHello (SNI) to a `proxy_tls` backend. Optional — needed only if the backend selects its certificate by SNI. **Requires `proxy_tls`**: without it there is no ClientHello to put a name in. Must be a **DNS hostname** per RFC 6066 §3 — ASCII letters, digits and hyphens in labels of 1–63, no empty or hyphen-edged label, no trailing dot, and **not an IP literal**. Omit the key to send no SNI at all; the `server_name` extension is then left out of the ClientHello entirely. |
