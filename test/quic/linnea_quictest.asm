...
; 1-RTT key update (RFC 9001 6.1). From an arbitrary traffic secret: the next
; secret is HKDF-Expand-Label(secret, "quic ku", "", 32) and the packet key and iv
; are re-derived from THAT. QUIC v2 (RFC 9369 3.3.2) renames all three labels, so
; each version has its own answers — deriving one under the other's labels yields
; keys the peer does not hold, and the connection dies at the first update.
; Expected values computed independently from RFC 8446 7.1 / RFC 9001 5.1.
ku_secret:      db 0x00,0x01,0x02,0x03,0x04,0x05,0x06,0x07,0x08,0x09,0x0a,0x0b,0x0c,0x0d,0x0e,0x0f
                db 0x10,0x11,0x12,0x13,0x14,0x15,0x16,0x17,0x18,0x19,0x1a,0x1b,0x1c,0x1d,0x1e,0x1f
exp_ku_v1_next: db 0x6a,0x4c,0xa3,0x49,0xa7,0x7d,0x86,0x43,0xfc,0x3d,0x19,0xb9,0x44,0xd2,0xc3,0xde
                db 0x71,0xcf,0xc6,0x72,0x7d,0xff,0x19,0x62,0xe0,0x0a,0x30,0xa6,0xc5,0xf4,0xa7,0xcd
exp_ku_v1_key:  db 0x52,0xa8,0xea,0x4d,0x44,0xbd,0x6e,0xe8,0xf7,0xf3,0x4d,0xf7,0x28,0xbe,0x5d,0xea
exp_ku_v1_iv:   db 0x8c,0x16,0x35,0xec,0xd2,0xb7,0x45,0xd8,0x46,0x71,0x58,0xed
exp_ku_v2_next: db 0x48,0xd4,0x7d,0x0f,0x9a,0xb5,0xb3,0x39,0x75,0x8d,0x7e,0x26,0x75,0x1d,0x94,0x25
                db 0xe1,0xd8,0x1b,0x7d,0x4c,0x21,0x6f,0x71,0x89,0x4f,0x38,0x3f,0xc9,0xb2,0x46,0xe9
exp_ku_v2_key:  db 0x83,0xbf,0xe0,0x19,0x2a,0xc3,0x1e,0x4a,0xf7,0xc6,0xe3,0x08,0xa8,0x23,0xb0,0x1e
exp_ku_v2_iv:   db 0xc6,0xfb,0x1b,0xc3,0x64,0x3c,0x14,0x4d,0x60,0xc8,0x02,0xfb
; a marker in the header-protection slot: an update must leave it alone (6.1)
ku_hp_mark:     db 0xa5,0xa5,0xa5,0xa5,0xa5,0xa5,0xa5,0xa5,0xa5,0xa5,0xa5,0xa5,0xa5,0xa5,0xa5,0xa5
...
.fl4_done:
    ...
    ; --- 1-RTT key update, under each version's labels ---
    xor r13d, r13d                   ; the version flag to test: v1, then v2
.ku_round:
    mov [quic_v2_active], r13b
    lea rdi, [ku_keys + linnea_quic_keys.hp]   ; a marker an update must not touch
    lea rsi, [ku_hp_mark]
    mov ecx, 16
    rep movsb
    lea rdi, [ku_secret]
    ...
    CHECK ku_keys + linnea_quic_keys.hp, ku_hp_mark, 16
    test r13d, r13d
    jnz .ku_v2
    CHECK ku_next_out, exp_ku_v1_next, 32
    CHECK ku_keys + linnea_quic_keys.key, exp_ku_v1_key, 16
    CHECK ku_keys + linnea_quic_keys.iv,  exp_ku_v1_iv,  12
    mov r13d, 1
    jmp .ku_round
.ku_v2:
    CHECK ku_next_out, exp_ku_v2_next, 32
