#!/usr/bin/env python3
"""Differential test: linnea's SHA-1 vs Python's hashlib over many random
inputs. Drives one `linnea-selftest sha1-stdin` process, streaming
length-prefixed frames and checking each returned digest.

Usage: diff_sha1.py [count]   (default 1_000_000)
...
def main():
    ...
    proc = subprocess.Popen([BIN, "sha1-stdin"],
